...
- هیرو خانه تمام‌عرض از بالای صفحه (پشت منوی شناور) است؛ پس‌زمینهٔ لوکال جواهرات روی پارچهٔ سفید و کارت `GlassSurface` همان شیشهٔ هدر.
...
پوستهٔ پنل مدیریت کلاس `admin-shell` است: شیشهٔ خنثی (لبه سفید/شب‌آبی، سایهٔ `primary` محو)،
پس‌زمینهٔ لکه‌های آبی و سبز ملایم، `--ring` روی `info`. طلایی برند در `/admin` به‌کار
نمی‌رود. سایت عمومی و داشبورد والد همان شیشهٔ طلایی می‌مانند.
